...
validate_dv <- function(x,strict=FALSE) {
    ...
    chk_df <- function(chk,msg,severity=1) data.frame(file_line_number=chk$file_line_number,message=msg,file_line=x$raw[chk$file_line_number],severity=severity,stringsAsFactors=FALSE)

    ## receive type must match serve type
    idx <- which(plays$skill %eq% "Reception")
    idx2 <- idx[!plays$skill[idx-1] %eq% "Serve"]
    if (length(idx2)>0) {
        out <- rbind(out,chk_df(plays[idx2,],"Reception was not preceded by a serve",severity=2))
    ...
    if (strict) {
        ...
        if (length(idx2)>0)
            out <- rbind(out,chk_df(plays[idx2,],paste0("Reception start zone (",plays$start_zone[idx2],") does not match serve start zone (",plays$start_zone[idx2-1],")"),severity=0))
        ...
        if (length(idx2)>0)
            out <- rbind(out,chk_df(plays[idx2,],paste0("Reception end zone (",plays$end_zone[idx2],") does not match serve end zone (",plays$end_zone[idx2-1],")"),severity=0))
        ## end zones mismatch, but not any missing
        idx2 <- idx[(!plays$end_subzone[idx] %eq% plays$end_subzone[idx-1]) & !is.na(plays$end_subzone[idx]) & !is.na(plays$end_subzone[idx-1])]
        if (length(idx2)>0)
            out <- rbind(out,chk_df(plays[idx2,],paste0("Reception end sub-zone (",plays$end_subzone[idx2],") does not match serve end sub-zone (",plays$end_subzone[idx2-1],")"),severity=0))
    }
    ...
    if (nrow(chk)>0)
        out <- rbind(out,chk_df(chk,"Player making a front row attack is in the back row",severity=2))
    ...
    if (any(chk))
        out <- rbind(out,data.frame(file_line_number=pp$file_line_number[chk],message="The listed player is not on court in this rotation",file_line=x$raw[pp$file_line_number[chk]],severity=2,stringsAsFactors=FALSE))
    ...
    if (any(chk))
        out <- rbind(out,data.frame(file_line_number=plays$file_line_number[chk],message="Point awarded to incorrect team following error (or \"error\" evaluation incorrect)",file_line=x$raw[plays$file_line_number[chk]],severity=2,stringsAsFactors=FALSE))
    ...
    chk <- (plays$skill %in% c("Serve","Attack","Block") & plays$evaluation_code %eq% "#") &  ## ace or winning attack or block
            (!plays$team %eq% plays$point_won_by)
    if (any(chk))
        out <- rbind(out,data.frame(file_line_number=plays$file_line_number[chk],message=paste0("Point awarded to incorrect team (or \"",plays$evaluation[chk],"\" evaluation incorrect)"),file_line=x$raw[plays$file_line_number[chk]],severity=2,stringsAsFactors=FALSE))
    ...
    for (k in idx) {
        ...
        if (!sub_out %in% prev_rot || !sub_in %in% new_rot || sub_in %in% prev_rot || sub_out %in% new_rot) {
            rot_errors[[length(rot_errors)+1]] <- data.frame(file_line_number=plays$file_line_number[k],message=paste0("Player lineup conflicts with recorded substitution: was the sub recorded incorrectly?"),file_line=x$raw[plays$file_line_number[k]],severity=2,stringsAsFactors=FALSE)
            next
        }
    }
